# Instagram Creative Bot — Worker 4.1
...
- **Референсы** — Worker использует встроенный утверждённый каталог из 473 RedGIFs-ниш, раздаёт локальному desktop задания на HOT-5, хранит складские сообщения и копирует их в группы моделей.
...
- отдельное меню «Включить / отключить ниши» глобально исключает ненужные ниши из сканирования, не стирая выбор ниш у групп моделей;
- сканер берёт только первые 5 результатов HOT: API всегда получает `order=hot`, fallback открывает `/niches/<slug>?order=hot`.
...
- `referenceProtocol: 5`
- `referenceScan.order: hot`
- `referenceScan.limit: 5`
