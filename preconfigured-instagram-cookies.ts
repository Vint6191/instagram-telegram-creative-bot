...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="creative-bot-") as temporary:
        ...
        cookies_file = resolve_cookies(workspace, config)
...
def load_config() -> dict[str, str]:
    ...
    if cookies_file:
        result["cookies_file"] = cookies_file
...
def resolve_cookies(workspace: Path, config: dict[str, str]) -> Path | None:
    configured_path = config.get("cookies_file")
    if configured_path:
        path = Path(configured_path)
        if not path.is_file():
            raise ConfigurationError("INSTAGRAM_COOKIES_FILE does not exist")
        content = path.read_bytes()
        validate_cookie_content(content)
        return path

    return write_cookies(workspace, config.get("cookies_b64"))
...
def validate_cookie_content(content: bytes) -> None:
    ...
    required = {b"sessionid", b"csrftoken", b"ds_user_id"}
    present: set[bytes] = set()
    for raw_line in content.splitlines():
        if not raw_line or raw_line.startswith(b"#"):
            continue
        columns = raw_line.split(b"\t")
        if len(columns) < 7:
            continue
        domain = columns[0].lstrip(b".").lower()
        if domain == b"instagram.com":
            present.add(columns[5])
    missing = sorted(required - present)
    if missing:
        names = ", ".join(item.decode("ascii", errors="replace") for item in missing)
        raise ConfigurationError(f"Instagram cookies are missing: {names}")
